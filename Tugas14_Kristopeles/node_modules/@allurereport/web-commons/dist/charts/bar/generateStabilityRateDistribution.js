...
export const generateStabilityRateDistribution = (options, storeData) => {
    const { title, threshold = 90 } = options;
    const { testResults } = storeData;
    const testResultsByFeature = new Map();
    ...
    return {
        data: Array.from(testResultsByFeature.entries()).map(([feature, { passed, total }]) => ({
            groupId: feature,
